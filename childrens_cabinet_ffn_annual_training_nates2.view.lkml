...
view: childrens_cabinet_ffn_annual_training_nates2 {
    ...
    sql_table_name: client_assessment_data;;
        ...
        dimension: c_Additional_Training {
            label: "Additional Training to Log?"
            group_label: "Questions"
            sql: (SELECT pl.value_name
                FROM picklist AS pl
                WHERE pl.field_name = 'c_Additional_Training'
                    AND pl.code <> ''
                    AND IF(COLUMN_GET(${TABLE}.custom_data, 'c_Additional_Training' AS INT) IS NULL, FALSE, 
                           pl.code = COLUMN_GET(${TABLE}.custom_data, 'c_Additional_Training' AS INT))
             )
          ;;
        }

        dimension: c_Additional_Training3 {
            label: "Additional Training to Log[3]"
            group_label: "Questions"
            sql: (SELECT pl.value_name
                FROM picklist AS pl
                WHERE pl.field_name = 'c_Additional_Training3'
                    AND pl.code <> ''
                    AND IF(COLUMN_GET(${TABLE}.custom_data, 'c_Additional_Training3' AS INT) IS NULL, FALSE, 
                           pl.code = COLUMN_GET(${TABLE}.custom_data, 'c_Additional_Training3' AS INT))
             )
          ;;
        }

        dimension: c_Additional_Training_2 {
            label: "Additional Training to Log [2]"
            group_label: "Questions"
            sql: (SELECT pl.value_name
                FROM picklist AS pl
                WHERE pl.field_name = 'c_Additional_Training_2'
                    AND pl.code <> ''
                    AND IF(COLUMN_GET(${TABLE}.custom_data, 'c_Additional_Training_2' AS INT) IS NULL, FALSE, 
                           pl.code = COLUMN_GET(${TABLE}.custom_data, 'c_Additional_Training_2' AS INT))
             )
          ;;
        }
