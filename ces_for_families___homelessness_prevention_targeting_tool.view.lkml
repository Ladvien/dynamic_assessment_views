...
view: ces_for_families___homelessness_prevention_targeting_tool {
	...
	dimension: id {
        ...
        label: "Id -- CES for Families - Homelessness Prevention Targeting Tool"
        primary_key: yes
        sql: ${TABLE}.id;;
    }
	...
	dimension: c_HoH_Under_Age_25 {
        label: "Head of household is under the age of 25"
        group_label: "Questions"
        sql: (SELECT pl.value_name
            FROM picklist AS pl
            WHERE pl.field_name = 'c_HoH_Under_Age_25'
                AND pl.code <> ''
                AND IF(COLUMN_GET(${TABLE}.custom_data, 'c_HoH_Under_Age_25' AS INT) IS NULL, FALSE, 
                       pl.code = COLUMN_GET(${TABLE}.custom_data, 'c_HoH_Under_Age_25' AS INT))
         );;
    }
