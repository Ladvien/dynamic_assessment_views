...
view: ces_for_individuals___ces_survey_part_1 {
	...
	dimension: id {
        label: "Id -- CES for Individuals - CES Survey Part 1"
        primary_key: yes
        sql: ${TABLE}.id;;
    }
	...
	dimension: c_Do_You_Have_Mental_Health_Condition {
        label: "Do you have or has anyone ever told you that you have a mental health condition?"
        group_label: "Questions"
        sql: (SELECT pl.value_name
            FROM picklist AS pl
            WHERE pl.field_name = 'c_Do_You_Have_Mental_Health_Condition'
                AND pl.code <> ''
                AND IF(COLUMN_GET(${TABLE}.custom_data, 'c_Do_You_Have_Mental_Health_Condition' AS INT) IS NULL, FALSE, 
                       pl.code = COLUMN_GET(${TABLE}.custom_data, 'c_Do_You_Have_Mental_Health_Condition' AS INT))
         );;
    }
	dimension: c_Serveyor_Detect_Symptoms {
        label: "OBSERVATION ONLY – DO NOT ASK:   Surveyor, do you detect signs or symptoms of severe, persistent mental illness or severely compromised cognitive functioning?"
        group_label: "Questions"
        sql: (SELECT pl.value_name
            FROM picklist AS pl
            WHERE pl.field_name = 'c_Serveyor_Detect_Symptoms'
                AND pl.code <> ''
                AND IF(COLUMN_GET(${TABLE}.custom_data, 'c_Serveyor_Detect_Symptoms' AS INT) IS NULL, FALSE, 
                       pl.code = COLUMN_GET(${TABLE}.custom_data, 'c_Serveyor_Detect_Symptoms' AS INT))
         );;
    }
	dimension: c_Staying_Self_Or_Others {
        label: "Are you staying by yourself or with a partner or other people?"
        group_label: "Questions"
        sql: (SELECT pl.value_name
            FROM picklist AS pl
            WHERE pl.field_name = 'c_Staying_Self_Or_Others'
                AND pl.code <> ''
                AND IF(COLUMN_GET(${TABLE}.custom_data, 'c_Staying_Self_Or_Others' AS INT) IS NULL, FALSE, 
                       pl.code = COLUMN_GET(${TABLE}.custom_data, 'c_Staying_Self_Or_Others' AS INT))
         );;
    }
	dimension: c_Housing_Seperate_From_Others {
        label: "Are you looking for housing that is separate from your partner or the other people you currently or used to stay with?"
        group_label: "Questions"
        sql: (SELECT pl.value_name
            FROM picklist AS pl
            WHERE pl.field_name = 'c_Housing_Seperate_From_Others'
                AND pl.code <> ''
                AND IF(COLUMN_GET(${TABLE}.custom_data, 'c_Housing_Seperate_From_Others' AS INT) IS NULL, FALSE, 
                       pl.code = COLUMN_GET(${TABLE}.custom_data, 'c_Housing_Seperate_From_Others' AS INT))
         );;
    }
	...
	dimension: c_Partner_Others_Threatened_You {
        label: "Has your partner or the other people you currently or used to stay with hurt or threatened you?"
        group_label: "Questions"
        sql: (SELECT pl.value_name
            FROM picklist AS pl
            WHERE pl.field_name = 'c_Partner_Others_Threatened_You'
                AND pl.code <> ''
                AND IF(COLUMN_GET(${TABLE}.custom_data, 'c_Partner_Others_Threatened_You' AS INT) IS NULL, FALSE, 
                       pl.code = COLUMN_GET(${TABLE}.custom_data, 'c_Partner_Others_Threatened_You' AS INT))
         );;
    }
	dimension: c_Refer_Planning_Safety {
        label: "Do you want me to refer you to someone you can talk to about planning for your safety?"
        group_label: "Questions"
        sql: (SELECT pl.value_name
            FROM picklist AS pl
            WHERE pl.field_name = 'c_Refer_Planning_Safety'
                AND pl.code <> ''
                AND IF(COLUMN_GET(${TABLE}.custom_data, 'c_Refer_Planning_Safety' AS INT) IS NULL, FALSE, 
                       pl.code = COLUMN_GET(${TABLE}.custom_data, 'c_Refer_Planning_Safety' AS INT))
         );;
    }
	dimension: c_Comfortable_With_HMIS {
        label: "Are you comfortable with your protected personal information being shared in HMIS with other organizations who participate in HMIS?"
        group_label: "Questions"
        sql: (SELECT pl.value_name
            FROM picklist AS pl
            WHERE pl.field_name = 'c_Comfortable_With_HMIS'
                AND pl.code <> ''
                AND IF(COLUMN_GET(${TABLE}.custom_data, 'c_Comfortable_With_HMIS' AS INT) IS NULL, FALSE, 
                       pl.code = COLUMN_GET(${TABLE}.custom_data, 'c_Comfortable_With_HMIS' AS INT))
         );;
    }
	...
	dimension: c_Staff_Observation_ADA_need_Communication_aides {
        label: "Staff Observation ADA need Communication aides"
        group_label: "Questions"
        sql: (SELECT pl.value_name
            FROM picklist AS pl
            WHERE pl.field_name = 'c_Staff_Observation_ADA_need_Communication_aides'
                AND pl.code <> ''
                AND IF(COLUMN_GET(${TABLE}.custom_data, 'c_Staff_Observation_ADA_need_Communication_aides' AS INT) IS NULL, FALSE, 
                       pl.code = COLUMN_GET(${TABLE}.custom_data, 'c_Staff_Observation_ADA_need_Communication_aides' AS INT))
         );;
    }
